(module
  ...
  (global $var_visible_1 (mut i32) (i32.const 1)) ;; 초시계
  ...
  (global $str_pool_ptr (mut i32) (i32.const 1168))
  ...
  (func $user_func_kr87 (param $entityIdx i32) (param $param_0 f64) 
    (local $temp f64)
    (local $condResult i32)
    (local $temp_str_ptr i32)
    
          ;; _if
          (if (f64.ne (local.get $param_0) (f64.const 1000000))
            (then 
          ;; change_variable: 변수
          (call $setVariable (i32.const 1144)
            (f64.add (call $getVariable (i32.const 1144)) (f64.const 1)))
          ;; Call function: func_0
          (call $user_func_kr87 (local.get $entityIdx) (f64.add (local.get $param_0) (f64.const 1)))))
  )
  
  ;; User function: func_1
  (func $user_func_rgpt (param $entityIdx i32) (param $param_0 f64) 
    (local $temp f64)
    (local $condResult i32)
    (local $temp_str_ptr i32)
    
          ;; _if
          (if (f64.ne (local.get $param_0) (f64.const 1000))
            (then 
          ;; move_x
          (call $setX (local.get $entityIdx)
            (f64.add (call $getX (local.get $entityIdx)) (f64.const 1)))
          ...
          (call $setRandomBrushColorInternal (local.get $entityIdx))
          ;; Call function: func_1
          (call $user_func_rgpt (local.get $entityIdx) (f64.add (local.get $param_0) (f64.const 1)))))
  )
      ...
      (block $done
        ...
        (block $pc_2
          ...
          (if (i32.eq (global.get $thread_0_loopCounter_0) (i32.const -1))
            (then
              (global.set $thread_0_loopCounter_0 (i32.trunc_f64_s (f64.const 400)))))
          ;; Check if iterations remain (counter > 0)
          (if (i32.gt_s (global.get $thread_0_loopCounter_0) (i32.const 0))
            (then
          ...
          ;; Call function: func_1
          (call $user_func_rgpt (i32.const 0) (f64.const 0))
          ;; move_y
          (call $setY (i32.const 0)
            (f64.add (call $getY (i32.const 0)) (f64.const -1)))
          (call $brushNotifyPosition (i32.const 0) (call $getX (i32.const 0)) (call $getY (i32.const 0)))
          ;; move_x
          (call $setX (i32.const 0)
            (f64.add (call $getX (i32.const 0)) (f64.const -1000)))
          (call $brushNotifyPosition (i32.const 0) (call $getX (i32.const 0)) (call $getY (i32.const 0)))
  ...
  (func $init
    ...
    (call $setX (i32.const 0) (f64.const -137.86))
    (call $setY (i32.const 0) (f64.const 93.69))
  ...
  (func $tick (param $dt f64)
    ...
    (global.set $str_pool_ptr (i32.const 1168))
