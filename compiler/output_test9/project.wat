(module
  ...
  (global $var_visible_0 (mut i32) (i32.const 1)) ;; 변수1
  (global $var_visible_1 (mut i32) (i32.const 1)) ;; 변수
  (global $var_visible_2 (mut i32) (i32.const 0)) ;; 초시계
  (global $var_visible_3 (mut i32) (i32.const 0)) ;; 대답
  ...
  (global $thread_0_loopCounter_1 (mut i32) (i32.const -1))
    ...
    (block $end
      (block $done
        (block $pc_0
          ...
          (if (i32.eqz (i32.gt_s (global.get $thread_0_resumeDepth) (i32.const 1)))
            (then
              (if (i32.eq (global.get $thread_0_loopCounter_0) (i32.const -1))
                (then
                  (global.set $thread_0_loopCounter_0 (i32.trunc_f64_s (call $getVariable (i32.const 1152))))))))
          ;; Check if iterations remain (counter > 0) OR resuming from deeper loop
          (if (i32.or (i32.gt_s (global.get $thread_0_resumeDepth) (i32.const 1)) (i32.gt_s (global.get $thread_0_loopCounter_0) (i32.const 0)))
            (then
              ;; Only decrement and execute preamble when NOT resuming from deeper
              (if (i32.eqz (i32.gt_s (global.get $thread_0_resumeDepth) (i32.const 1)))
                (then
                  (global.set $thread_0_loopCounter_0
                    (i32.sub (global.get $thread_0_loopCounter_0) (i32.const 1)))
                  ))
              ;; Always execute from nested loop onwards
              
          ;; repeat_while_true (mode: while, EntryJS-compatible: one iteration per tick, depth 1)
          (if (f64.ne (call $getVariable (i32.const 1152)) (f64.const 20))
            (then
              
          ;; change_variable: 변수
          (call $setVariable (i32.const 1152)
            (f64.add (call $getVariable (i32.const 1152)) (f64.const 1)))
          ;; change_variable: 변수1
          (call $setVariable (i32.const 1144)
            (f64.add (call $getVariable (i32.const 1144)) (f64.const 10)))
              ;; Add tiny delay and stay at same PC for next iteration
              (global.set $thread_0_resumeDepth (i32.const 2))
              (global.set $thread_0_waiting (f64.const 0.001))
              (return (i32.const 1))))
          ;; Condition no longer met - fall through to advance PC
          ;; set_variable: 변수
          (call $setVariable (i32.const 1152) (f64.const 10))
  ...
  (func $tick (param $dt f64)
    ...
    (if (i32.and
      ...
      (then
        (global.set $thread_0_loopCounter_0 (i32.const -1))
        (global.set $thread_0_loopCounter_1 (i32.const -1))
        (global.set $thread_0_resumeDepth (i32.const 0))
