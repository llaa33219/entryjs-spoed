(module
  ...
  (global $str_pool_ptr (mut i32) (i32.const 1144))
  ...
  ;; User function: 이동하고 회전
  (func $user_func_moveAndRotate (param $entityIdx i32) (param $param_0 f64) (param $param_1 f64) 
    (local $temp f64)
    (local $condResult i32)
    (local $temp_str_ptr i32)
          ...
          (call $brushNotifyPosition (local.get $entityIdx) (call $getX (local.get $entityIdx)) (call $getY (local.get $entityIdx)))
          ;; rotate_relative
          (call $setRotation (local.get $entityIdx)
            (f64.add (call $getRotation (local.get $entityIdx)) (local.get $param_1)))
  )
  
  ;; User function: 두 배 계산
  (func $user_func_calcDouble (param $entityIdx i32) (param $param_0 f64) (result f64)
    (local $temp f64)
    (local $condResult i32)
    (local $temp_str_ptr i32)
    
    ;; Return value
    (f64.mul (local.get $param_0) (f64.const 2))
  )
    ...
    (block $end
      (block $done
        (block $pc_0
          ...
          ;; Call function: 앞으로 이동
          (call $user_func_moveForward (i32.const 0) (f64.const 50))
        ...
        (block $pc_2
          ...
          ;; Call function: 앞으로 이동
          (call $user_func_moveForward (i32.const 0) (f64.const 100))
          (local.set $pc (i32.add (local.get $pc) (i32.const 1)))
          (global.set $thread_0_pc (local.get $pc))
          (global.set $thread_0_resumeDepth (i32.const 0))
          (br $done))
    ...
    (block $end
      (block $done
        (block $pc_0
          ...
          (if (i32.eq (global.get $thread_1_loopCounter_0) (i32.const -1))
            (then
              (global.set $thread_1_loopCounter_0 (i32.trunc_f64_s (f64.const 5)))))
          ;; Check if iterations remain (counter > 0)
          (if (i32.gt_s (global.get $thread_1_loopCounter_0) (i32.const 0))
            (then
              ;; Decrement counter first (like EntryJS)
              (global.set $thread_1_loopCounter_0
                (i32.sub (global.get $thread_1_loopCounter_0) (i32.const 1)))
              ;; Execute inner blocks
              
          ;; Call function: 이동하고 회전
          (call $user_func_moveAndRotate (i32.const 0) (f64.const 20) (f64.const 15))
  ...
  (func $tick (param $dt f64)
    ...
    (global.set $str_pool_ptr (i32.const 1144))
    ...
    ;; Activate thread 1 on key 32 (entity scene: 0)
    (if (i32.and
