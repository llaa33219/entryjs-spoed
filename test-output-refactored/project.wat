(module
  ...
  ;; Get entity base offset: 1024 + entityIndex * 64
  (func $getEntityOffset (param $idx i32) (result i32)
    (i32.add
      (i32.const 1024)
      (i32.mul (local.get $idx) (i32.const 64))))
    ...
    (block $end
      (block $done
        (block $pc_0
          ...
          ;; _if
          (if (f64.gt (call $getX (i32.const 0)) (f64.const 200))
            (then 
          ;; locate_xy
          (call $setX (i32.const 0) (f64.const -200))
          (call $setY (i32.const 0) (call $getY (i32.const 0)))))
          ;; change_to_next_shape
          (call $setPictureIndex (i32.const 0)
            (i32.add (call $getPictureIndex (i32.const 0)) (i32.const 1)))
          ;; Reset PC to repeat
  ...
  (func $thread_1_run (param $entityIdx i32) (result i32)
    ...
    (block $end
      (block $done
        (block $pc_0
          (br_if $pc_0 (i32.ne (local.get $pc) (i32.const 0)))
          ;; change_variable: score
          (call $setVariable (i32.const 1088)
            (f64.add (call $getVariable (i32.const 1088)) (f64.const 1)))
          (local.set $pc (i32.add (local.get $pc) (i32.const 1)))
  ...
  (func $init
    ...
    (call $setScaleX (i32.const 0) (f64.const 0.5))
    (call $setScaleY (i32.const 0) (f64.const 0.5))
    ...
    (call $setVariable (i32.const 1088) (f64.const 0))
  ...
  (func $tick (param $dt f64)
    ...
    ;; Activate thread 0 on start
    (if (i32.eqz (global.get $frameCount))
      (then (global.set $thread_0_active (i32.const 1))))
    ;; Activate thread 1 on key 32
    (if (call $isKeyPressed (i32.const 32))
